...
// Renders a single configured field as an input control bound to a row value.
...
function FormCard({ item, canEdit, onDelete }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await updateRequirement(item.id, { title, content: { fields, rows } });
  ...
  return (
    <div className="rounded-lg border border-cream-300 p-4 dark:border-slate-700">
      ...
      {canEdit && (
        <div className="mt-3 flex justify-end gap-2 border-t border-cream-200 pt-3 dark:border-slate-700">
          <button className="pill-delete" onClick={onDelete}>Delete Form</button>
...
// Tab 2 — Custom Input Forms: one or more config-driven forms per project.
export default function FormTab({ projectId, canEdit }) {
  ...
  return (
    <div className="space-y-4">
      {canEdit && <button className="btn-secondary" onClick={add}>+ Add Form</button>}
      {items.length === 0 ? (
        <p className="text-sm text-slate-400">No custom forms yet.</p>
      ) : (
        items.map((it) => <FormCard key={it.id} item={it} canEdit={canEdit} onDelete={() => remove(it)} />)
      )}
    </div>
  );
}
